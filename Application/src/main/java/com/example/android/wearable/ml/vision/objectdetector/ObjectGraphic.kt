...
package com.example.android.wearable.ml.vision.objectdetector
...
class ObjectGraphic constructor(
    ...
    override fun draw(canvas: Canvas) {
        ...
        val RuleOfThirds_rect = Rect(
            Math.round((canvas.width / 3).toFloat()), //left
            Math.round((canvas.height / 3).toFloat()), //top
            Math.round((canvas.width * 2 / 3).toFloat()), //right
            Math.round((canvas.height * 2 / 3).toFloat()) // bottom
        )

        //Draw RuleOfThirds lines
        canvas.drawLine(
            0f, RuleOfThirds_rect.top.toFloat(),
            canvas.width.toFloat(), RuleOfThirds_rect.top.toFloat(),
            boxPaints[0]
        )

        canvas.drawLine(
            0f, RuleOfThirds_rect.bottom.toFloat(),
            canvas.width.toFloat(), RuleOfThirds_rect.bottom.toFloat(),
            boxPaints[0]
        )

        canvas.drawLine(
            RuleOfThirds_rect.left.toFloat(), 0f,
            RuleOfThirds_rect.left.toFloat(), canvas.height.toFloat(),
            boxPaints[0]
        )

        canvas.drawLine(
            RuleOfThirds_rect.right.toFloat(), 0f,
        ...
        val zLevel = if (direction == Direction.NONE) {
            ZLevel.LEVELED
        ...
        viewModel.putSmartCompositionDirection(direction, ZLevel.LEVELED)
    }
